...
class AIPromptProcessor {
  ...
  detectComplexity(prompt) {
    ...
    const lowerPrompt = prompt.toLowerCase();
    if (
      complexityKeywords.simple.some((keyword) => lowerPrompt.includes(keyword))
    ) {
      return 0.2;
    }
    if (
  ...
  detectStructure(prompt) {
    if (
      prompt.toLowerCase().includes("verse") ||
      prompt.toLowerCase().includes("chorus")
    ) {
      return "song";
    }
    if (
  ...
  generateMIDISequence(analysis, options = {}) {
    ...
    const sequence = [];
    const scale =
      this.musicTheoryKnowledge.scales[analysis.scale] ||
      this.musicTheoryKnowledge.scales.major;
    const keyOffset = this.getKeyOffset(analysis.key);
    const baseNote = 60 + keyOffset; // Middle C + key offset

    // Generate chord progression
    const progressions =
      this.musicTheoryKnowledge.chordProgressions[analysis.genre] ||
    ...
    for (let i = 0; i < length; i++) {
      ...
      let note;
      if (Math.random() < 0.7) {
        // 70% chance for chord tone
        note = baseNote + scale[chordRoot] + octave * 12;
      } else {
        // 30% chance for scale tone
...
if (require.main === module) {
  ...
  async function test() {
    const analysis = await processor.processPrompt(
      "Create a happy jazz chord progression in C major"
    );
